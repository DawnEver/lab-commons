...
| [Testing discipline](testing-discipline.md) | The lightweight/heavy partition, xfail never skip, one pytest session at a time |
...
| [Alignment](alignment.md) | The framework every branch develops against so an integration conflicts on CODE and never on bookkeeping |
...
| [Killed runs and orphans](orphans.md) | What survives a stopped run, why a lock does not time out, and the box-level census a reaper needs |
| [Box resources](box-resources.md) | What one workstation rations, the four defects measured in doing it by hand, and the broker's shape |
| [The forge](forge.md) | How `main` is protected on a self-hosted forge: the push whitelist, the server-side status check, branch disposal |
...
| [The docs pipeline](docs-pipeline.md) | The three properties a docs builder must hold, and why an unbuilt sub-site must be announced rather than silent |
