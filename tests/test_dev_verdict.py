...
class TestTheSelector:
    def test_a_selector_that_expands_to_nothing_is_refused(self) -> None:
        """Finding nothing must never be the same value as finding nothing wrong."""
        with pytest.raises(ValueError, match='expands to nothing'):
            Selector(spec='gate', node_ids=())

    def test_a_selector_with_no_spec_is_refused(self) -> None:
        with pytest.raises(ValueError, match='no spec'):
            Selector(spec='   ', node_ids=('t/a.py::test_one',))

    def test_the_expansion_is_named_and_sorted(self) -> None:
...
class TestTheProof:
    ...
    def test_a_silent_node_makes_the_proof_incomplete_and_is_named(self, selector) -> None:
    ...
    def test_a_node_that_reported_without_being_selected_makes_it_incomplete(self, selector) -> None:
    ...
    def test_a_truncation_makes_it_incomplete_and_names_itself(self, selector) -> None:
    ...
    def test_a_skip_is_silent_not_reported(self, selector) -> None:
    ...
    def test_a_complete_proof_refuses_nothing(self, proof) -> None:
        assert proof.refusal == ''
        assert proof.shortfall == ()


class TestPromotionIsRefusedWithoutProof:
    def test_two_covered_nodes_and_a_clean_proof_is_a_pass(self, proof) -> None:
        settled = Result.settled(proof=proof)
        assert settled.outcome is Outcome.PASS
        assert not settled.failures

    def test_SETTLED_DERIVES_FAIL_FROM_THE_FAILURES_NAMED(self, proof) -> None:
    ...
    def test_a_pass_cannot_be_constructed_without_a_proof(self) -> None:
    ...
    def test_a_pass_cannot_be_constructed_over_a_partial_proof(self, selector) -> None:
        partial = Proof.of(selector, ('t/a.py::test_one',))
        with pytest.raises(IncompleteRun, match='silent'):
            Result.settled(proof=partial)

    def test_a_fail_needs_a_proof_as_much_as_a_pass_does(self) -> None:
    ...
    def test_a_fail_that_names_no_failure_is_refused(self, proof) -> None:
    ...
    def test_a_failure_that_never_reported_is_refused(self, proof) -> None:
        with pytest.raises(IncompleteRun, match='never reported'):
            Result.settled(proof=proof, failures=('t/a.py::test_absent',))

    def test_a_pass_carrying_failures_is_refused(self, proof) -> None:
...
class TestTheInconclusiveDefault:
    def test_inconclusive_needs_no_proof(self) -> None:
    ...
    def test_inconclusive_must_say_why(self) -> None:
        """Cannot-say is only information once it says WHAT it cannot say."""
        with pytest.raises(ValueError, match='no reason'):
            Result.inconclusive('  ')

    def test_inconclusive_may_carry_the_incomplete_proof_as_evidence(self, selector) -> None:
        partial = Proof.of(selector, ('t/a.py::test_one',))
        result = Result.inconclusive(partial.refusal, proof=partial)
        assert result.proof is partial

    def test_inconclusive_OVER_A_COMPLETE_PROOF_is_refused(self, proof) -> None:
    ...
    def test_inconclusive_cannot_carry_failures(self) -> None:
    ...
    def test_the_rendered_line_is_the_outcome_and_the_proof_is_elsewhere(self) -> None:
...
class TestTheVerdict:
    def test_a_verdict_carries_all_five_and_renders_them(self, selector, proof, log) -> None:
        verdict = Verdict(
    ...
    def test_a_verdict_that_names_no_tree_is_refused(self, selector, proof, log) -> None:
    ...
    def test_a_verdict_that_names_no_environment_is_refused(self, selector, proof, log) -> None:
        with pytest.raises(ValueError, match='name the tree'):
            Verdict(tree='sha256:deadbeef', env='', selector=selector, result=Result.settled(proof=proof), log=log)

    def test_stamping_writes_the_verdict_into_its_own_log(self, selector, proof, log) -> None:
        verdict = Verdict(
    ...
    def test_stamping_refuses_when_the_log_moved_under_the_verdict(self, selector, proof, log) -> None:
        """THE MID-RUN MOVE.

        The limitation the ``HEAD + "-dirty"`` stamp was measured to have: the lines the verdict
        read are no longer the lines on disk.
        """
        verdict = Verdict(
            tree='sha256:deadbeef', env='c0ffee', selector=selector, result=Result.settled(proof=proof), log=log
        )
    ...
    def test_stamping_TOLERATES_a_log_that_grew_after_the_reference_was_taken(self, selector, proof, log) -> None:
        """The asymmetry that makes stamping possible at all.

        The verdict line is APPENDED to the log it describes, so growth past the covered prefix cannot invalidate it
        -- and a digest of
