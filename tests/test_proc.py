...
class TestSystemMemory:
    def test_this_box_is_readable(self) -> None:
    ...
    def test_used_is_derived_not_read_twice(self) -> None:
    ...
    def test_reports_bytes_not_gigabytes(self) -> None:
...
class TestWorkingSet:
    def test_reads_our_own_process(self) -> None:
        held = working_set_bytes(os.getpid())
        assert held is not None
        assert held > 0

    def test_unreadable_pid_is_none_not_zero(self) -> None:
...
class TestLiveness:
    ...
    def test_a_nonexistent_pid_is_not_alive(self) -> None:
...
class TestDescendants:
    ...
    def test_collects_the_whole_tree(self) -> None:
    ...
    def test_a_pid_absent_from_the_table_has_no_tree(self) -> None:
        assert descendants(99, {10: ('root.exe', 1)}, lambda _pid: None) == frozenset()

    def test_refuses_an_edge_whose_child_predates_its_parent(self) -> None:
    ...
    def test_keeps_an_edge_with_equal_creation_stamps(self) -> None:
    ...
    def test_terminates_on_a_forged_cycle(self) -> None:
        rows = {10: ('a.exe', 11), 11: ('b.exe', 10)}
        assert descendants(10, rows, lambda _pid: None) == frozenset({10, 11})


class TestProcessTree:
    def test_unreadable_table_is_empty_never_just_the_root(self, monkeypatch) -> None:
    ...
    def test_includes_ourselves_on_a_readable_box(self) -> None:
        assert os.getpid() in process_tree(os.getpid())


class TestKill:
    def test_killing_a_nonexistent_pid_reports_failure(self) -> None:
    ...
    def test_killing_an_unreadable_tree_reaps_nothing(self, monkeypatch) -> None:
        monkeypatch.setattr('lab_commons.proc._process_rows', lambda: None)
        assert kill_process_tree(424242) == frozenset()

    def test_kills_descendants_before_the_root(self, monkeypatch) -> None:
    ...
    def test_reports_only_what_it_confirmed(self, monkeypatch) -> None:
