...
class TestWhatTheAddressSees:
    ...
    def test_the_path_is_part_of_the_record_not_only_the_bytes(self, tree):
...
class TestWhatTheAddressIsBlindTo:
    def test_an_unmeasured_file_does_not_move_the_address(self, tree):
        before = content_address(tree, MEASURED)
        (tree / 'docs' / 'notes.md').write_text('rewritten prose\n', encoding='utf-8')
        (tree / 'docs' / 'new.md').write_text('a new page\n', encoding='utf-8')
        assert content_address(tree, MEASURED) == before

    def test_an_interpreter_cache_inside_a_measured_dir_does_not_move_the_address(self, tree):
        """The suite rewrites ``__pycache__`` by RUNNING, so measuring it would make an unchanged
        tree address differently for the sole reason that it was measured."""
        before = content_address(tree, MEASURED)
    ...
    def test_the_address_is_a_fact_about_LAYOUT_not_about_where_the_tree_sits(self, tree, tmp_path):
...
class TestTheRefusals:
    def test_a_target_outside_the_root_is_refused(self, tree, tmp_path):
    ...
    def test_addressing_nothing_is_refused_rather_than_folded_into_a_constant(self, tree):
        """ "The tree did not change" and "I measured no tree" must not be the same string."""
        with pytest.raises(ValueError, match='nothing to address'):
            content_address(tree, ())

    def test_a_zero_length_address_is_refused(self, tree):
        with pytest.raises(ValueError, match='floor'):
            content_address(tree, MEASURED, length=0)

    def test_file_digest_is_the_content(self, tree):
