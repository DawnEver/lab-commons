...
def _profile(checkout, **overrides):
    fields = {
        'app_name': 'widget',
        'package': 'widget',
        'root': checkout,
...
class TestResolvingTheRoot:
    def test_a_declared_root_is_the_root(self, checkout):
        assert _profile(checkout).repo_root() == checkout.resolve()

    def test_a_declared_root_that_is_not_a_directory_is_refused(self, checkout):
        with pytest.raises(NotACheckout, match='not a directory'):
            _profile(checkout, root=checkout / 'nowhere').repo_root()

    def test_the_home_environment_variable_outranks_a_detected_anchor(self, checkout, tmp_path, monkeypatch):
    ...
    def test_an_anchor_inside_the_checkout_finds_the_checkout(self, checkout, monkeypatch):
    ...
    def test_with_no_root_no_home_and_no_anchor_it_refuses_rather_than_guessing(self, monkeypatch, mocker):
    ...
    def test_the_derived_trees_hang_off_the_root(self, checkout):
...
class TestTheLintConfigLocation:
    ...
    def test_an_absolute_override_is_honoured(self, checkout, tmp_path):
        other = tmp_path.parent / 'shared-ruff.toml'
        assert _profile(checkout, lint_config=other).lint_config_path == other


class TestExemptions:
    def test_an_exempt_directory_covers_everything_under_it(self, checkout):
        declared = _profile(checkout)
        assert declared.is_exempt(checkout / 'attic' / 'old.py')

    def test_a_path_that_merely_STARTS_with_an_exempt_name_is_not_exempt(self, checkout):
    ...
    def test_an_exemption_carries_its_reason(self, checkout):
        declared = _profile(checkout)
    ...
    def test_an_exemption_with_no_reason_is_refused(self, checkout):
    ...
    def test_a_path_outside_the_repo_cannot_be_named_relative_to_it(self, checkout, tmp_path):
        with pytest.raises(ValueError, match='not inside'):
            _profile(checkout).relative(tmp_path.parent / 'elsewhere.py')


class TestPinsAndNames:
    def test_a_pin_is_a_named_set_not_a_count(self, checkout):
        """An integer cannot say WHICH row moved, and the honest repair when it disagrees is to
        edit the digit -- which has already happened once in this family."""
        assert _profile(checkout).pin('suppressions') == frozenset({'src/widget/core.py'})

    def test_an_undeclared_pin_reports_nothing_rather_than_raising(self, checkout):
    ...
    def test_a_pin_with_no_name_is_refused(self, checkout):
        with pytest.raises(ValueError, match='no name'):
            _profile(checkout, pins={'  ': frozenset({'x'})})

    def test_a_package_that_is_not_importable_is_refused(self, checkout):
        with pytest.raises(ValueError, match='not an importable'):
            _profile(checkout, package='src/widget')

    def test_an_app_name_that_is_empty_is_refused(self, checkout):
