...
def _worker_ok(x: int, y: int) -> dict:
    return {'solution_type': 'OK', 'value': x + y}


def _worker_boom(x) -> Never:
    msg = f'boom {x}'
    raise ValueError(msg)


def test_machine_cores_is_a_positive_int() -> None:
    assert isinstance(machine_cores, int)
    assert machine_cores >= 1


def test_default_params_timeout_is_60s() -> None:
...
def test_params_accept_a_time_quantity() -> None:
...
def test_params_field_itself_is_permissive_but_m_as_s_enforces_the_dimension() -> None:
    """`get_quantity_type`'s field validator only requires *a* pint Quantity.

    See ``test_units.py::test_pydantic_quantity_rejects_a_raw_non_quantity_at_the_class_level``
...
def test_run_maps_task_over_args_list_across_processes() -> None:
...
def test_run_accepts_kwds_list_alone() -> None:
    mp_run = MultiProcessing(
        _worker_ok,
...
def test_run_num_process_zero_returns_empty_without_spawning() -> None:
    mp_run = MultiProcessing(_worker_ok, args_list=[[1, 2]], num_process=0)
    assert mp_run.run() == []


def test_init_raises_without_args_or_kwds() -> None:
    with pytest.raises(ParameterException):
        MultiProcessing(_worker_ok)


def test_init_num_process_capped_by_task_count() -> None:
    mp_run = MultiProcessing(_worker_ok, args_list=[[1, 2]], num_process=machine_cores)
    assert mp_run._num_process == 1


def test_run_propagates_a_worker_exception_in_debug_mode() -> None:
    """`_try_get_result`'s `debug=True` branch does not catch worker errors.

    Documented behaviour, not the TIMEOUT/ERROR-tagging retry path, and pinned here so a future
    change to that flag is a deliberate one.
    """
